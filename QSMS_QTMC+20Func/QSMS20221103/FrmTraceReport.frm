...
Begin VB.Form frmTraceReport 
   ...
   Begin VB.Label Label6 
      BackColor       =   &H00C0C0FF&
      Caption         =   "If you choose the txt format, the data result storage path: C:\TraceReportData\ The data result sharing path: D:\TraceReportData\"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
...
End If
    ...
    If UCase(Trim(CbbDataType.text)) = "TRACE BY COMPPN" Then
        ...
        If DateDiff("d", DTPBeginDate.Value, DTPEndDate.Value) > 31 Then
            MsgBox "The time span of one query of the system cannot exceed 31 days, please split the query multiple times."
            Exit Sub
        End If
    ...
    Else
        ...
        lblInfor.Caption = "from server:" & Trim(Rs.Fields("FilePath")) & " copy TraceReport data to local:" & strDestPath & ",please check!"
        strSourceFile = Trim(Rs.Fields("FilePath"))
        FileCopy strSourceFile, strDestPath
        
        MsgBox "from server:" & Trim(Rs.Fields("FilePath")) & " copy TraceReport data to local:" & strDestPath & ", please check!", vbOKOnly, "Message"
