...
Dim RS As New ADODB.Recordset
Dim str As String
Dim TempDateTime, TempGroupDatetime As String
Dim Response As String
...
'''''''''''''''''''''''''''''''''''''begin (0005)
    ...
    Set RS = Conn.Execute(str)
    If UCase(RS!Item) = "N" Then
        MsgBox "The GroupID is over 4 week,can not insert"
        Exit Sub
    End If
'''''''''''''''''''''''''''''''''''''end (0005)
For I = 0 To lstWO_SELECT.ListCount - 1
...
str = "select top 1 Group_TransDateTime as BeginDateTime, dbo.FormatDate(DATEADD(MONTH,1,dbo.Format_To_Date(Group_TransDateTime)),'YYYYMMDDHHNNSS') as EndDateTime from QSMS_WoGroup where GroupID = '" & Trim(CboGroupID) & "' order by Group_TransDateTime"
If RS.State = 1 Then RS.Close
Set RS = Conn.Execute(str)
