...
Begin VB.Form FrmReturnDID 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin TabDlg.SSTab sstabDID 
         ...
         Tab(0).Control(0)=   "DGDIDReturned"
         Tab(0).Control(1)=   "DGDIDNeedReturned"
         Tab(0).Control(2)=   "DGDIDInfo"
...
DoPrintLabel:
                        ...
                        '*Denver        2009.08.16     NB2&NB3分家，但是用同一个QSMS可作NB2NB3 的退料
...
'        strsql = "select DID,RealQty from QSMS_DID WHERE DID = (select DID from QSMS_DID where NextDID='" & Trim(TxtDID) & "') AND RealQty>0"
        ...
        If RS.EOF = False Then
            ...
            Set RS = Conn.Execute(strSQL)
            MsgBox "此DID有接料且没用完！" + vbCrLf + "前一个DID为：" + PreDID + ";数量为:" + CStr(RestQty) + "." + vbCrLf + "后一个DID为：" + RS!DID + ";数量为:" + CStr(RS!realqty) + "."
        End If
    End If
    '1162
    ...
    Set RS = Conn.Execute(strSQL)
    If RS.EOF = False Then
        MsgBox "此DID的材料是独用料,请确认Return的数量是否和实际数量一致！"
    End If
    ...
    If RS.EOF = False Then
        ...
        If TotalQty = TxtReturnQty Then    ''''1197
          MsgBox "此DID尚未在产线使用过，请再三检查是否真的需要Return!!!"
        End If
    End If
   ''1221
