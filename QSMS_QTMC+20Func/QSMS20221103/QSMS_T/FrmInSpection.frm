...
'  Dim TestType As srting
 ...
 '''==============重测并清除以前测试数据================
  ...
  If UCase(Trim(lblunit)) = "IC" Then
        If MsgBox("请核对IC 信息是否正确?", vbYesNo, "IC 测试") = vbYes Then
            TestStr = 0   ''''0--> pass  ,1---> fail
            Errcode = "PASS"
        Else
            TestStr = 1
            Errcode = InputBox("请输入错误代码：", "IC 测试", "")
  ...
  ElseIf UCase(Trim(lblunit)) = "CON" Then
     If MsgBox("Connecter PIN:" & lblSpec & "  请核对实物是否相符?", vbYesNo, "CON 测试") = vbYes Then
            TestStr = 0   ''''0--> pass  ,1---> fail
            Errcode = "PASS"
     Else
            TestStr = 1
            Errcode = InputBox("请输入错误代码：", "CON 测试", "")
            strSQL = "selecT * from IPQC_ErrCode where ErrCode='" & Errcode & "'   "
            If RS.State Then RS.Close
            RS.Open strSQL, Conn
            If RS.EOF Then
            ...
            Else
                ...
                End If
                  If DataGrid1.Columns(4) = "CON" Or DataGrid1.Columns(4) = "IC" Then
                      lblMsg = txtDID & "目检结果为 ：" & IPQCFlag & "，目检完成！ "
                  Else
                      lblMsg = txtDID & "测试结果为 ：" & IPQCFlag & "，测试完成！ "
                  End If
                  txtDID.Enabled = True
                  txtDID = ""
                  cmdStart.Enabled = False
...
If KeyAscii = vbKeyReturn Then
 ...
 ''''get rule of comppn
    ...
    If UCase(Trim(lblunit)) = "R" Or UCase(Trim(lblunit)) = "C" Or UCase(Trim(lblunit)) = "L" Or UCase(Trim(lblunit)) = "Z" Then
        If MsgBox("请核对DID\CompPN信息是否正确？", vbYesNo, "提示信息") <> vbYes Then
           Call txtDID_Click
           Exit Sub
        End If
    End If
...
Private Function chkIPQCRetest(UID As String, DID As String) As Boolean
   ...
   Dim strSQL As String
     ...
     RS.Open strSQL, Conn
     If RS.EOF = False Then
        If MsgBox("确定要重测该DID吗？", vbYesNo, "Message") = vbYes Then
