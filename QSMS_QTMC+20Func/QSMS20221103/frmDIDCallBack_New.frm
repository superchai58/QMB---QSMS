...
Private Function GetGroupWO(ByVal GroupID As String)
    sSql = "select Work_Order,ClosedFlag from QSMS_WOGroup with(nolock) where GroupID= '" & GroupID & "'"
    Set Rst = Conn.Execute(sSql)
...
Private Function GetLine()
    sSql = "select distinct Line from QSMS_WOGroup with(nolock) order by line"
    Set Rst = Conn.Execute(sSql)
    CboLine.Clear
    Do While Rst.EOF = False
...
'    sEDate = Replace(Replace(dtpEDate, "/", ""), "-", "")
    ...
    If OptRelease.Value = True Then
       sSql = "select distinct GroupID from QSMS_WOGroup with(nolock) where WO_TransDateTime between  '" & sSDate & "' and '" & sEDate & "' and line='" & CboLine & "' and closedflag<>'Y'"
    Else
        sSql = "select distinct GroupID from QSMS_WOGroup with(nolock) where substring(Group_TransDateTime,1,8) between '" & sSDate & "' and '" & sEDate & "' and line='" & CboLine & "' and closedflag<>'Y'"
    End If
...
Private Function GetDIDInfoCallBack(ByVal WO As String)
    ...
    sSql = "select DID,CompPN,TotalQty,ReturnQty,Transdatetime,UID from QSMS_DIDCallBack with(nolock) where Work_Order in(Select WO from dbo.GetWOGroup(" & WO & ")) and ReturnFlag='Y' order by DID"
...
'    sSql = "select DID,CompPN,TotalQty,ReturnQty from QSMS_GroupDID where GroupID='" & GroupID & "' and ReturnFlag<>'Y' Order by DID"
    sSql = "select DID,CompPN,TotalQty,ReturnQty from QSMS_DIDCallBack with(nolock) where Work_Order in(Select WO from dbo.GetWOGroup(" & WO & ")) and ReturnFlag<>'Y' Order by DID"
    ...
    sSql = "select A.DID,A.CompPN,A.Qty as TotalQty,A.RemainQty,A.RealQty,A.VendorCode,A.LotCode,A.UID,A.Transdatetime,A.UsedFlag,A.Inheritflag from QSMS_DID A with(nolock),QSMS_DIDCallBack B with(nolock) where A.DID=B.DID and B.Work_Order in(Select WO from dbo.GetWOGroup(" & WO & ")) and B.ReturnFlag<>'Y' Order by A.DID"
