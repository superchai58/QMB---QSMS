...
Private Function ValidateData() As Boolean
    ...
    'If Len(Trim(TxtCompPN)) <> 11 Or Len(Trim(TxtCompPN)) <> 15 Then
    '    LblMessage = "The CompPN's length must be = 11 or 15!!"
    '    Exit Function
    'End If
    
    If Len(Trim(TxtCompPN)) <> 11 Then
        If Len(Trim(TxtCompPN)) <> 15 Then
            LblMessage = "The CompPN's length must be = 11 or 15!!"
            Exit Function
        End If
    End If
...
Dim DIDTypeMSD As String        'superchai Add 20230720
...
On Error GoTo errHandler
        ...
        '--superchai Add 20230720 (B)--
        strSQL = "Exec QSMS_GetDIDMSD @DID='" & Trim(TxtCompPN.text) & "'"
        Set rsTime = Conn.Execute(strSQL)
        If rsTime.EOF = False Then
            If rsTime("result") = "1" Then
               DIDTypeMSD = "MSD"
            End If
        End If
        '--superchai Add 20230720 (E)--
...
Private Sub reFreshData()
    ...
    'sSql = "select top 800 * from CompPrintLog order by TransDateTime desc" '20231216 Warinya CompPrint not shoow 15 QPN
    sSql = "select top 800 * from CompPrintLog with(nolock) where CompPN not in (select distinct NewQPN from QSMS_CompPNTransfer with(nolock)) order by TransDateTime desc"
...
'    Sendkeys "{home}+{end}"        'superchaiComments
End Sub
...
If KeyAscii = 13 And Trim(TxtCompPN) <> "" Then
    ...
    ElseIf InStr(1, Trim(TxtCompPN.text), ";") > 0 Then
        ...
        '20231216 Warinya CompPrint no need to check
        'sSql = "select NewQPN, Vendor from QSMS_CompPNMapping with(nolock) where OldQPN = '" & Trim(TxtCompPN.text) & "'"
        'Set Rs = Conn.Execute(sSql)
        'If Not Rs.EOF Then
...
'        Call TxtQty_Click
    ...
    '                Exit Function
            ...
            'If Len(Trim(TxtCompPN)) <> 11 Then
            '    If Len(Trim(TxtCompPN)) <> 15 Then
            '   LblMessage.Caption = "The CompPN's length must be = 11 or 15 !!"
            '    Exit Sub
            'End If
...
Private Sub TxtDateCode_Click()
   'Sendkeys "{home}+{end}"     'superchaiComment
End Sub
...
Private Sub TxtLotCode_Click()
    'Sendkeys "{home}+{end}"        'superchaiComemnt
End Sub
