...
Dim k As Integer
    ...
    If (CboReportType.text = "DispatchDID") Then
        Call GetLine
    End If
...
Select Case CboReportType
        ...
        ''''1279
            ...
            If Not tmpRS.EOF Then
                If MsgBox(Trim(tmpRS("Descr")), vbOKCancel, "Tip") <> vbOK Then
                    Exit Sub
                End If
            End If
...
While Not Rs.EOF
    If (CboReportType.text = "DispatchDID") Then
        Strsql1 = "Exec GetPlant2Line '" & plant & "','" & Trim(Rs!Line) & "'"
...
Else
    If InStr(1, Jobpn, "-") > 0 Then
       ...
       '--superhai comments (Begin) 20230516--
       TempJobPn = Mid(Jobpn, 1, 11)
       TxtMBPN = TempJobPn
       TxtRev.text = Right(Jobpn, 3)
       '--superhai comments (End) 20230516--
       
       '--superchai add new function from QSMC (Begin) 20230516--
       ...
       'TxtMBPN = TempJobPn
       'TxtRev.text = Right(Jobpn, 3)
       '--superchai add new function from QSMC (End) 20230516--
    Else
      TempJobPn = Jobpn
    End If
    If OptRelease.Value = True Then
...
If Trim(CboJobPN) <> "" Then
   TempJobPn = Mid(CboJobPN, 1, 11)  'superchai comments QMB0003 20230516
   
   '--superchai add function from QSMC (Begin) QMB0003 20230516--
   '20230508 Kaelyn forPN э20絏
    'str = "SELECT CompPN FROM QSMS_DID with(nolock) where DID='" & CboJobPN & "'"
    'Set Rs = Conn.Execute(str)
    'TempJobPn = CStr(Rs!COMPPN)
   '--superchai add function from QSMC (End) QMB0003 20230516--
End If
...
'  str = "Select * from QSMS_WO with(nolock) where work_order='" & Trim(Work_Order) & "' order by jobpn,machine,slot,lr,comppn"
   str = "Exec QSMSRpt_ME_BOM_WO '" & Work_Order & "'"
   Set Rs = Conn.Execute(str)
   If Not Rs.EOF Then
       Call CopyToExcel(Rs)
   Else
...
'             End If
       ...
       Case "By_JobPN"
             ...
             If CboJobPN = "" Then
                 ...
                 Exit Function
             Else
                'Jobpn = Mid(CboJobPN, 1, 11) superchai comments QMB0004 20230516
                
                'superchai add function from QSMC (Begin) QMB0004 20230516--
                '20230508 Kaelyn forPN э20絏
                str = "SELECT CompPN FROM QSMS_DID with(nolock) where DID='" & CboJobPN & "'"
                Set Rs = Conn.Execute(str)
                Jobpn = CStr(Rs!COMPPN)
                'superchai add function from QSMC (End) QMB0004 20230516--
             End If
...
'             End If
       ...
       Case "By_JobPN"
             ...
             If CboJobPN = "" Then
                 ...
                 Exit Function
             Else
                'Jobpn = Mid(CboJobPN, 1, 11) superchai comments QMB0005 20230516
                
                '--superchai add function from QSMC (Begin) QMB0005 20230516--
                '20230508 Kaelyn forPN э20絏
                str = "SELECT CompPN FROM QSMS_DID with(nolock) where DID='" & CboJobPN & "'"
                Set Rs = Conn.Execute(str)
                Jobpn = CStr(Rs!COMPPN)
                '--superchai add function from QSMC (End) QMB0005 20230516--
             End If
             str = "exec QSMSRptPrepareMaterialByJobPN '" & WO & "','" & Jobpn & "'"
