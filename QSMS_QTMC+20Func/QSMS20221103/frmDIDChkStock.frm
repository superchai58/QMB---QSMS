...
Private Sub CmdQuery_Click()
    ...
    End If
    ''Stephen 分廠區之line
    sSql = "exec XL_CheckRefID @BeginDate=" & sq(BeginDate) & ",@EndDate=" & sq(EndDate) & ",@plant=" & sq(plant)
    Set rstCompPN = Conn.Execute(sSql)
    If rstCompPN.EOF = False Then
        If rstCompPN("Result") <> 0 Then
            MsgBox rstCompPN("Description"), vbExclamation, "Prompt"
...
Private Sub txtRefID_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 And Trim(txtRefID) <> "" Then
        ...
        If chkStock.Value = 0 Then
            lblMsg = "ReferenceID:" & Trim(txtRefID) & " ToWH info is below:"
            sSql = "exec XL_DIDChkStockByRefID @Type='Query',@RefID=" & sq(Trim(txtRefID)) '& ",@BeginDate=" & sq(BeginDate) & ",@EndDate=" & sq(EndDate)
        Else
            sSql = "exec XL_DIDChkStockByRefID @Type='Manual',@RefID=" & sq(Trim(txtRefID)) & ",@UserName=" & sq(g_userName) ' & ",@BeginDate=" & sq(BeginDate) & ",@EndDate=" & sq(EndDate)
        End If
