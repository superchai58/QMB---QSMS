...
Begin VB.Form frmMaintainDIDAutoDispatch 
   Caption         =   "Maintain DID & Auto dispatch  20240125-1"
   ClientHeight    =   9495
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15660
...
Dim DIDTypeMSD As String 'Stephen 20220802
...
Dim str09Codetmp As String '1290
Dim Rs As New ADODB.Recordset
Dim str As String, i As Integer, IsBSMaterial As String

'Dim BarCode As String  ''001401
'Dim UniqueID As String ''001401
BarCode = "" ''001401
UniqueID = "" ''001401
...
'Stephen 依廠區insert line
If optToWO.Value = True And previousCompPN <> CboCompPN.text Then
    'clear line
    For i = CmbLine.ListCount - 1 To 0 Step -1
       CmbLine.RemoveItem i
    Next i
End If
...
'            If GetTickCount - strDelaytime > 10000000 Then
...
If (KeyAscii = 13 Or KeyAscii = 9) And CboCompPN <> "" Then
    If Chk2DCode = "Y" Then '''1295
        str = "exec Check2DCode" & Trim(sq(CboCompPN.text))
    ...
    End If
        If UCase(Trim(StrBU)) = "NB4" Then
            CboCompPN.text = UCase(Replace(Replace(Replace(CboCompPN.text, " ", ""), vbCr, ""), vbLf, "")) ''1303
        Else
            BarCode = CboCompPN.text ''001401
            CboCompPN.text = UCase(Replace(Replace(Replace(Replace(CboCompPN.text, " ", ""), vbCr, ""), vbLf, ""), ":", ";")) ''1247
    End If
    
'    If IsTrayComp = 0 And DateDiff("s", PrintTime, Now) <= AutoDispatchTimeSpan Then
'        lblstatus.Caption = "Please wait " & AutoDispatchTimeSpan & " sec before continuing to operate the AutoDispatch."
'        Call ClearData
'        Exit Sub
'    End If
    ...
    If InStr(1, Trim(CboCompPN.text), ";") > 0 Then  '(1020)
        ...
        For index = 0 To UBound(NewComp)
            ...
            ElseIf index = 6 Then ''001401
                UniqueID = Trim(NewComp(index)) ''001401
            End If
                
        Next index
        
        If UCase(Trim(StrBU)) = "NB4" Then  '(1303)
            If Not InStr(1, Trim(TxtLotCode.text), "-") > 0 Then
                MsgBox ("BinCode is null!"), vbCritical
                Exit Sub
            End If
        End If
        
        ''''''''''''''''001401''''''''''''''''
'        If UCase(Trim(StrBU)) = "ESBU" Then
'            str = "exec XL_DIDSaveDIDLabel_Data '" & Trim(CboCompPN.Text) & "','" & Trim(txtDateCode.Text) & "','" & Trim(CboVendorCode.Text) & "','" & Trim(txtLotCode.Text) & "','" & Trim(TxtQty.Text) & "','" & Trim(UniqueID) & "','" & Trim(BarCode) & "'"
'            Set RS = Conn.Execute(str)
'            If RS.EOF = True Then
'                MsgBox ("解析记录UniqueID和2DBarCode出现保错!"), vbCritical
'            End If
'        End If
        ''''''''''''''''001401''''''''''''''''
        ...
        If CheckBSMaterial = "Y" Then   ''(1213)
            ...
            If TxtLotCode = "@@@@" Then
                IsBSMaterial = "Y"
                                 '''''''''''''''''''''1290'''''''''''''''''''''''''''''''''''
                TxtQty.text = Trim(Replace(TxtQty.text, "PCS", ""))
                If Chk09Code = "Y" Then
                    str09Code = InputBox("Please Input 09Code:     ", "Input 09Code")
                    If Mid(Trim(CboCompPN.text), 1, 1) <> "A" Then
                        str09Codetmp = Trim(NewComp(5))
                        If Len(str09Code) >= 20 Then
                            If Len(str09Codetmp) = 8 Then
                               If Trim(str09Codetmp) <> Mid(Trim(str09Code), 3, 8) Then
                                  MsgBox ("09Code not match!"), vbCritical
                                  Call Warning_Sound
                                  Call ClearData
                                  Exit Sub
                               End If
                            Else
                                If Mid(Trim(str09Codetmp), 2, 8) <> Mid(Trim(str09Code), 3, 8) Then
                                     MsgBox ("09Code not match!"), vbCritical
                                     Call Warning_Sound
                                     Call ClearData
                                     Exit Sub
                        ...
                        If Len(str09Code) < 20 Then
                             If Len(str09Codetmp) = 8 Then
                                 If Trim(str09Codetmp) <> Mid(Trim(str09Code), 2, 8) Then
                                      MsgBox ("P/N not match!"), vbCritical
                                      Call Warning_Sound
                                      Call ClearData
                                      Exit Sub
                                 End If
                             Else
                                 If Mid(Trim(str09Codetmp), 2, 8) <> Mid(Trim(str09Code), 2, 8) Then
                                      MsgBox ("P/N not match!"), vbCritical
                                      Call Warning_Sound
                                      Call ClearData
                                      Exit Sub
        ...
        If IsBSMaterial = "Y" Then   ''1208
            TxtDateCode.text = ""
            TxtLotCode.text = ""
            TxtDateCode.text = UCase(Trim(InputBox("Please input Datecode:", "Input Datecode")))
            TxtLotCode.text = UCase(Trim(InputBox("Please input Lotcode:", "Input Lotcode")))
        End If
    ...
    End If
    '''(0060)
     If CheckBCMS = "Y" Then '1296
                                                                  
         str = "select * from BCMS_Bios where chippn like '%" & Trim(CboCompPN.text) & "%'"
         Set Rs = Conn.Execute(str)
         If Rs.EOF = False Then
            str = "select * from UserRight  where appname='QSMS' and Userright='CheckBiosLogin' and  UserName='" & g_userName & "'"
            Set Rs = Conn.Execute(str)
            If Rs.EOF = True Then
                MsgBox (g_userName & "无发Bois材料的权限，请Check!"), vbCritical
                CboCompPN_Click
                Call Warning_Sound
                Call ClearData
                Exit Sub
         ...
         End If
    End If
    If OneByOneControl = "Y" Then '1297
         str = "select * from OneToOneControl where comppn like '%" & Trim(CboCompPN.text) & "%'"
         Set Rs = Conn.Execute(str)
         If Rs.EOF = False Then
            MsgBox ("请注意，此材料需一对一管控!"), vbCritical
         End If
    End If
    If ChkVendorCode = "Y" Then
        str = "exec QSMS_VendorCode_Check '" & Trim(CboCompPN.text) & "','" & Trim(CboVendorCode.text) & "'"
        Set Rs = Conn.Execute(str)
        If Trim(Rs!result) <> "1" Then
          MsgBox ("Message: " & Rs!ErrDesc), vbCritical
          CboCompPN_Click
...
'    End If
    ...
    If IsTrayComp = 1 Then '''1251
        ...
        If Trim(Rs!result) <> "1" Then
            ...
            Exit Sub
        Else
            TxtGroupQty.text = Int(Trim(TxtQty.text)) / Int(Trim(Rs!Qty))
            TxtQty.text = Trim(Rs!Qty)
            TxtGroupQty.SetFocus
        End If
        
        If CLng(TxtGroupQty.text) < 1 Then
            MsgBox ("Message: GroupQty must be an integer."), vbCritical
            CboCompPN_Click
            Call EffectSound("OO.wav")
            Call ClearData
            Exit Sub
...
If UCase(Trim(CheckNeedMSD)) = "Y" Then   ''(1228)
    If IsNeedMSD(CboCompPN) = True Then
        MsgBox "This is MSD material, please bake it first.！"
    End If
End If
...
If KeyAscii = 13 Or KeyAscii = 9 Then
     ...
     End If
'     If CheckQty.Value = 1 Then '0081
'         Call TxtQty_KeyPress(13)
'
'     Else
'         TxtQty.SetFocus
'     End If
    If CheckQty.Value = 1 Then  ''勾选
        TxtQty.SetFocus
    Else
        Call TxtQty_KeyPress(13)  ''若"Check Qty" 選項不打勾,立即产生DID
    End If
End If
    ...
    Else
        ...
        If plant = "TH20" Then
            strSQL = "Select top 150 DID,CompPN,VendorCode,DateCode,LotCOde,Qty,UID,remainQty,TransDateTime,Line,Side,FirstMachine,UsedFlag From QSMS_DID Where CompPN like '" & Trim(StrPN) & "' and DID like '" & Trim(CboDID) & "%' " & _
                " and line in (select line from TH20Plant with(nolock)) Order by TransDateTime desc "
            Set rs2 = Conn.Execute(strSQL)
            If Not rs2.EOF Then Set DG1.DataSource = rs2
        ElseIf plant = "TH2C" Then
            strSQL = "Select top 150 DID,CompPN,VendorCode,DateCode,LotCOde,Qty,UID,remainQty,TransDateTime,Line,Side,FirstMachine,UsedFlag From QSMS_DID Where CompPN like '" & Trim(StrPN) & "' and DID like '" & Trim(CboDID) & "%' " & _
                    " and line in (select line from TH2CPlant with(nolock)) Order by TransDateTime desc "
                Set rs2 = Conn.Execute(strSQL)
                If Not rs2.EOF Then Set DG1.DataSource = rs2
        Else
            strSQL = "Select top 150 DID,CompPN,VendorCode,DateCode,LotCOde,Qty,UID,remainQty,TransDateTime,Line,Side,FirstMachine,UsedFlag From QSMS_DID Where CompPN like '" & Trim(StrPN) & "' and DID like '" & Trim(CboDID) & "%' " & _
...
On Error GoTo errhandle
    ...
    str = "exec [QSMS_ReprintLock] @DID='" & Trim(CboDID) & "', @UID='" & Trim(g_userName) & "'"
    Set Rs = Conn.Execute(str)
    If Rs.EOF = False Then
        If Trim(Rs!result) <> "0" Then
            MsgBox Trim(Rs!Message)
            Exit Sub
        End If
    End If
    ...
    'stephen XL_GetDidPrintInfo_T test for annotation delete
    ...
    ElseIf UCase(Trim(Rs!FirstMachine)) = "DATECODE_CHECK" Then  ''(0079)
       MsgBox "DID DateCode is greater than the defined DateCode, please confirm."
       CboDID.SetFocus
       Exit Sub
    ...
    Else
        If UCase(Trim(StrBU)) = "ESBU" Then
            If UCase(Trim(Rs!DateCodeFlag)) = "FAIL" Then  ''(0079)
        ...
        If AutoDispatchPrintlable = "Y" Then        '1300
            PrintData.Work_Order = Trim(Rs!Work_Order) '1300
            PrintData.ModelName = Trim(Rs!ModelName) '1300
        End If
...
'    cmdAdd.Enabled = True
    ...
    Else ''(0061)
        If ChkOldDIDLabelQty <> "" And opOldLabel.Value = True Then
            ...
            End If
        Else
            If IsTrayComp = 0 Then
                If CInt(Trim(TxtGroupQty)) > CInt(chkQty) And CheckQty.Value = 0 Then
 ...
 If StrBU = "NB1" Then  '(1402)
    strSQL = "select 0 from Mapping_VendorPN where QuantaPN='" & Trim(CboCompPN.text) & "'"
     Set Rs = Conn.Execute(strSQL)
     If Not Rs.EOF Then
        ''弹出输入框
        VendorPN = InputBox("请输入对应厂商料号", "")
        strSQL = "select 0 from Mapping_VendorPN where QuantaPN='" & Trim(CboCompPN.text) & "' and VendorPN='" & Trim(VendorPN) & "'"
        Set Rs = Conn.Execute(strSQL)
        If Rs.EOF Then
            MsgBox ("广达料号与厂商料号不对应!"), vbCritical
            Call Warning_Sound
            Call ClearData
            Exit Sub
       End If
 ...
 End If
    ...
    If optExtra = True Or optSpecial = True Or optToWO = True Then
        ...
        If txtExtraQty = "" Then 'QMS Stephen
            txtExtraQty = 0
        End If
...
If Chk09Code = "Y" And str09Code <> "" Then  ''1290
      strSQL = "Select 0 From ConsignPN_HUA Where [TYPE] = 'HUACustomerPN' and CompPN='" & CboCompPN.text & "'"
      Set Rs = Conn.Execute(strSQL)
      If Rs.EOF Then
           strSQL = "Insert into ConsignPN_HUA(TYPE,CompPN,TransDateTime,UID) values('HUACustomerPN','" & CboCompPN.text & "',[DBO].[FormatDate](getdate(), 'YYYYMMDDHHNNSS'),'" & Trim(g_userName) & "')"
           Set Rs = Conn.Execute(strSQL)
      End If
      If Len(str09Code) < 20 Then ''小于20码表示国内生产,没有09Code,不需要Link 09Code
         str09Code = ""
      End If
   End If
    strStep = "2"
            ...
            For i = 1 To CLng(Trim(TxtGroupQty))
                ...
                Do While Not InsertDIDOk And RetryCnt < 10
                    ...
                    Else
                        ...
                        If UCase(Trim(StrBU)) = "ESBU" Then
                            strVendorCode = Trim(Rs!VendorCode) '1282
                        End If
                    ...
                    ''''''''''''''''001401''''''''''''''''
'                    If UCase(Trim(StrBU)) = "ESBU" Then
                        str = "exec XL_DIDSaveDIDLabel_Data '" & Trim(CboCompPN.text) & "','" & Trim(TxtDateCode.text) & "','" & Trim(CboVendorCode.text) & "','" & Trim(TxtLotCode.text) & "','" & Trim(TxtQty.text) & "','" & Trim(UniqueID) & "','" & Trim(BarCode) & "','" & Trim(TempDID) & "'"
                        Set Rs = Conn.Execute(str)
                        If Rs.EOF = True Then
                            MsgBox ("An error occurred while parsing UniqueID and 2D Barcode."), vbCritical
                        End If
'                    End If
                    ''''''''''''''''001401''''''''''''''''
...
Retry:
                ...
                If NewcompFlag = True And UCase(Trim(StrBU)) = "NB6" Then    'save 2dBarCode DID for trace（0082）
                strSQL = "Exec DIDTrace_SaveData @DID='" & Trim(TempDID) & "', @Type='F'"
                    Set Rs = Conn.Execute(strSQL)
                End If
                
                If ReturnDID = True And UCase(Trim(StrBU)) = "NB6" Then    'save DID for trace（0082）
'                strsql = "Exec DIDTrace_SaveData @DID='" & Trim(TempDID) & "', @Type='F'"
                 strSQL = "Exec DIDTrace_SaveReturnData @DID='" & Trim(TempDID) & "', @FDID=" & sq(OldReturnDID) & ",@Type='R'"
                    Set Rs = Conn.Execute(strSQL)
                End If
                ...
                If PreDIDPrinted <> CboDID And InsertDIDOk = True Then    '(0021)  'add Insertdidok=true (0040)
                    ...
                    If DispatchCompPrint = "Y" Then '1287
                        If CompPrint.Value = 1 Then
                            Call PrintLabelCompPrint
                        End If
                    End If
                End If
...
End If
End Sub
Private Function PrintLabelCompPrint()
Dim hFile As Long
Dim i As Integer
...
'If UCase(Trim(StrBU)) = "NB5" Then '（0081）
'    CheckQty.Visible = True
'    CheckQty.Enabled = True
'Else '（0081）
'    CheckQty.Visible = False
'    CheckQty.Enabled = False
'End If
...
PrintTime = DateAdd("s", -AutoDispatchTimeSpan, Now)
...
Dim str As String
'On Err GoTo errHandler:        'superchai comments 20240125
...
Exit Function
'errHandler:
'    MsgBox Err.Description     'superchai comments 20240125
End Function
...
'    strDay = Format(Now, "YYYY/MM/DD")
    ...
    '0080
    If opOldLabel.Value = True And UCase(Trim(StrBU)) = "ESBU" Then
        TrayStr = "Exec QSMS_GetRecordTray @DID='" & Trim(strDID) & "',@UID='" & g_userName & "'"        '''(0056)
        Set tmpRS = Conn.Execute(TrayStr)
    End If
    '0080
...
'    End If
    ...
    'Stephen 20220802
     tmpStr = "Exec QSMS_GetDIDMSD @DID='" & Trim(strDID) & "'"
     Set tmpRS = Conn.Execute(tmpStr)
     If tmpRS.EOF = False Then
...
'    Close #hFile
    ...
    If InStr(tmpPrintStr, "<DID_2D>") > 0 Then
     ...
     If PrintData.Work_Order <> "" Then
        tmpPrintStr = Replace(tmpPrintStr, "<VendorCode>", strVendorCode)     ''1282
        
        ''QMS Stephen 20220715
        tmpPrintStr = Replace(tmpPrintStr, "<Work_Order>", PrintData.Work_Order)     ''1301
        
     End If
     tmpPrintStr = Replace(tmpPrintStr, "<UniqueID>", PrintData.UniqueID) 'Stephen add Quanta Delivery Label 1286
...
'    If CboCompPN.text = "" Then
'        MsgBox "Please input CompPN first"
'        optToWO.Value = False
'        Exit Sub
'    End If
...
If KeyAscii = 13 And TxtGroupQty <> "" Then
    ...
    Else ''(0061)
        If ChkOldDIDLabelQty <> "" And opOldLabel.Value = True Then
            ...
            End If
        Else
'           If CInt(Trim(TxtGroupQty)) > CInt(chkQty) And CheckQty.Value = 0 Then '（0081）
'                MsgBox ("Max reel Qty must be less than " & ChkQty & " !"), vbCritical
'                Call Warning_Sound
'                Call ClearData
'                Exit Sub
'            End If
            If CInt(Trim(TxtGroupQty)) > CInt(chkQty) And CheckQty.Value = 1 Then '（0081）
                MsgBox ("Max reel Qty must be less than " & chkQty & " !"), vbCritical
                Call Warning_Sound
                Call ClearData
                Exit Sub
...
'    cmdAdd.SetFocus        ''''(0055)
'If IsInteger(TxtQty) = False And CheckQty.Value = 0 Then
'        MsgBox ("please check the qty of print, Qty>0!"), vbCritical
'        TxtQty.SetFocus
'        Call Warning_Sound
'        Call ClearData
'        Exit Sub
    If IsInteger(TxtQty) = False And CheckQty.Value = 1 Then
        MsgBox ("please check the qty of print, Qty>0!"), vbCritical
        TxtQty.SetFocus
        Call Warning_Sound
        Call ClearData
...
If (KeyAscii = 13 Or KeyAscii = 9) Then
    ...
    End If
'    If CheckQty.Value = 1 Then '（0081）
'        If TxtGroupQty.Text = "" Then
'            MsgBox ("请输入GroupQty !"), vbCritical
'            Exit Sub
'        End If
'        Call TxtGroupQty_KeyPress(13) '（0081）
'    Else
'        TxtGroupQty.SetFocus
'        Call TxtGroupQty_Click
'    End If
